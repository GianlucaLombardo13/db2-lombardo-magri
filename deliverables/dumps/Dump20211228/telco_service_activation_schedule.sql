...
CREATE TABLE `service_activation_schedule` (
  ...
  `order` int NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_service_activation_schedule_1_idx` (`order`),
  CONSTRAINT `fk_service_activation_schedule_1` FOREIGN KEY (`order`) REFERENCES `order` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
